...
const EVE_COMPACT_SESSION_ROUTE_PATH = `${EVE_ROUTE_PREFIX}/session/compact`;
const EVE_CLEAR_SESSION_ROUTE_PATH = `${EVE_ROUTE_PREFIX}/session/clear`;
...
async function openStreamBody(input) {
	...
	for (let attempt = 0; attempt < openRetryPolicy.maxAttempts; attempt += 1) {
		const url = createClientUrl(input.host, createEveMessageStreamRoutePath(input.sessionId), Object.keys(searchParams).length > 0 ? searchParams : void 0);
...
var ClientSession = class {
	...
	async snapshot(options) {
		...
		const continuationToken = lastEvent?.type === "session.waiting" ? lastEvent.data.continuationToken : lastEvent?.type === "session.completed" && this.#context.preserveCompletedSessions ? initialState.continuationToken : void 0;
		return {
			events,
			session: continuationToken === void 0 ? {
				sessionId,
	...
	async clear() {
		...
		if (continuationToken === void 0) {
			if (state.sessionId !== void 0) throw new Error("Session has no continuation token. Consume its event stream before clearing.");
		...
		if (!result.success || result.data.status === "accepted" && state.sessionId !== void 0 && result.data.sessionId !== state.sessionId) throw new Error(`Clear route returned an invalid response (${response.status}).`);
	...
	async compact() {
		...
		const continuationToken = state.continuationToken;
		if (continuationToken === void 0) {
			if (state.sessionId !== void 0) throw new Error("Session has no continuation token. Consume its event stream before compacting.");
			return { status: "no_active_session" };
		}
		...
		if (!result.success || result.data.status === "accepted" && state.sessionId !== void 0 && result.data.sessionId !== state.sessionId) throw new Error(`Compact route returned an invalid response (${response.status}).`);
		return result.data.status === "accepted" ? {
			sessionId: result.data.sessionId,
			status: "accepted"
		} : { status: "no_active_session" };
