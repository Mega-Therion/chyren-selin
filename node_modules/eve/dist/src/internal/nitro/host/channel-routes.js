...
`)].join(`
`)}function addChannelCorsPreflightHandler(e,t){if(t.preflightRoutes.has(t.route))return;t.preflightRoutes.add(t.route);let n=`OPTIONS ${t.route}`,r=`${EVE_CHANNEL_VIRTUAL_ID_PREFIX}${n}`;e.options.handlers.push({handler:r,method:`OPTIONS`,route:t.route}),e.options.virtual[r]=[`import { handleCors } from ${t.nitroH3ModulePath};`,`const cors = ${JSON.stringify(t.cors)};`,`export default (event) => {`,`  const corsResponse = handleCors(event, cors);`,`  if (corsResponse !== false) return corsResponse;`,`  return new Response(null, { status: 204 });`,`};`].join(`
