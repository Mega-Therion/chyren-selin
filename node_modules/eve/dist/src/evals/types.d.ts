...
export interface EveEvalAssertions {
    ...
    event<TType extends MessageStreamEvent["type"]>(type: TType, options?: Omit<Extract<EveEvalEventMatch, {
        type: TType;
    }>, "type">): AssertionHandle;
    notEvent<TType extends MessageStreamEvent["type"]>(type: TType, options?: Omit<Extract<EveEvalEventMatch, {
        type: TType;
    }>, "type" | "count">): AssertionHandle;
    eventOrder(matchers: readonly EveEvalEventMatch[]): AssertionHandle;
    eventsSatisfy(label: string, predicate: (events: readonly MessageStreamEvent[]) => boolean): AssertionHandle;
}
...
export type EveEvalStreamEvent<TType extends MessageStreamEvent["type"] = MessageStreamEvent["type"]> = Extract<MessageStreamEvent, {
    type: TType;
}>;
/** Matcher options for waiting until one live turn emits a specific event. */
export type EveEvalWaitForEventOptions<TType extends MessageStreamEvent["type"]> = Omit<Extract<EveEvalEventMatch, {
    type: TType;
}>, "count" | "type">;
/**
...
export interface EveEvalLiveTurn {
    ...
    waitForEvent<TType extends MessageStreamEvent["type"]>(type: TType, options?: EveEvalWaitForEventOptions<TType>): Promise<EveEvalStreamEvent<TType>>;
}
...
export interface EveEvalTargetHandle extends EveEvalTarget {
     ...
     */
    watchTurn(sessionId: string, opts?: {
        readonly startIndex?: number;
    }): EveEvalLiveTurn;
}
/**
